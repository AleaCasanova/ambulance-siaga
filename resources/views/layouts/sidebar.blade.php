...
<aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
    ...
    <nav class="px-4 py-5 space-y-1.5 overflow-y-auto flex-1">
        ...
        @if($user && $user->isAdmin())
            <div class="px-3 mb-2 text-[10px] font-extrabold text-slate-500 uppercase tracking-wider">Admin Module</div>
        ...
        @if($user && $user->isOperator())
            <div class="px-3 mb-2 text-[10px] font-extrabold text-slate-500 uppercase tracking-wider">Operator Module</div>
            <div class="space-y-1">
                <!-- Order Masuk -->
                <a href="{{ route('operator.orders.index') }}"
                   class="flex items-center gap-3 px-3.5 py-2.5 rounded-lg text-sm font-medium transition-colors {{ request()->routeIs('operator.orders.*') ? 'bg-red-600 text-white shadow-md shadow-red-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
                ...
                <a href="{{ route('operator.monitoring') }}"
                   class="flex items-center gap-3 px-3.5 py-2.5 rounded-lg text-sm font-medium transition-colors {{ request()->routeIs('operator.monitoring') ? 'bg-red-600 text-white shadow-md shadow-red-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
                    <svg class="w-5 h-5 opacity-80" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @if(!$user || $user->isMasyarakat())
            ...
            <a href="{{ route('masyarakat.info') }}"
               class="flex items-center gap-3 px-3.5 py-2.5 rounded-lg text-sm font-medium transition-colors {{ request()->routeIs('masyarakat.info') ? 'bg-red-600 text-white shadow-md shadow-red-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
                ...
                <span>Tentang Ambulance Siaga</span>
            </a>
        @endif
    ...
    <div class="px-4 py-3 border-t border-slate-800/60 bg-slate-950/80 text-[11px] text-slate-500 text-center flex-shrink-0 font-medium">
        Ambulance Siaga &copy; {{ date('Y') }} • v1.0
    </div>
