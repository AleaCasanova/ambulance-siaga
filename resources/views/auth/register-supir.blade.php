    ...
    <div class="w-full max-w-[460px] sm:max-w-[500px] lg:max-w-[520px] my-auto">
        ...
        <div class="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-5 lg:p-6 shadow-[0_12px_35px_-8px_rgba(14,138,205,0.12)] border border-slate-100/90">
            ...
            <div class="mb-3 text-center">
                ...
                <h2 class="text-xl sm:text-2xl font-black text-[#0F2747] tracking-tight">Daftar Pengemudi</h2>
                <p class="text-[11px] sm:text-xs font-semibold text-slate-500 mt-0.5 leading-relaxed max-w-sm mx-auto">
                    Bergabung bersama jaringan Ambulance Siaga. Akun Anda akan diverifikasi oleh Admin sebelum aktif.
                </p>
            </div>

            <form method="POST" action="{{ route('register.supir') }}" class="space-y-2">
                @csrf
                ...
                <div class="grid grid-cols-2 gap-2">
                    <!-- Field: Nomor SIM -->
                    <div>
                        <label for="nomor_sim" class="block text-[10px] font-extrabold text-slate-700 uppercase tracking-wider mb-0.5">
                            Nomor SIM (A/B)
                        </label>
                        <input id="nomor_sim" type="text" name="nomor_sim" :value="old('nomor_sim')" required
                               placeholder="contoh: 123456789012"
                               class="w-full px-3 py-1.5 sm:py-2 rounded-lg border border-slate-200 bg-slate-50/60 text-xs font-semibold text-[#0F2747] placeholder:text-slate-400 focus:bg-white focus:border-[#1E9DE5] focus:ring-2 focus:ring-[#1E9DE5]/20 transition-all outline-none">
                        <x-input-error :messages="$errors->get('nomor_sim')" class="mt-0.5 text-[11px]" />
                    </div>

                    <!-- Field: Plat Nomor -->
                    <div>
                        <label for="plat_nomor" class="block text-[10px] font-extrabold text-slate-700 uppercase tracking-wider mb-0.5">
                            Plat Nomor Kendaraan
                        </label>
                        <input id="plat_nomor" type="text" name="plat_nomor" :value="old('plat_nomor')" required
                               placeholder="contoh: R 1234 AB"
                               class="w-full px-3 py-1.5 sm:py-2 rounded-lg border border-slate-200 bg-slate-50/60 text-xs font-semibold text-[#0F2747] placeholder:text-slate-400 focus:bg-white focus:border-[#1E9DE5] focus:ring-2 focus:ring-[#1E9DE5]/20 transition-all outline-none">
                        <x-input-error :messages="$errors->get('plat_nomor')" class="mt-0.5 text-[11px]" />
                    </div>
                </div>

                <div class="grid grid-cols-2 gap-2">
                    <!-- Field 4: Kata Sandi (Password) with Eye Icon Toggle -->
                    ...
                    <!-- Field 5: Konfirmasi Kata Sandi with Eye Icon Toggle -->
