    ...
    <div class="w-full max-w-[460px] sm:max-w-[500px] lg:max-w-[520px] my-auto">
        ...
        <div class="bg-white rounded-2xl sm:rounded-3xl p-5 sm:p-6 lg:p-7 shadow-[0_12px_35px_-8px_rgba(14,138,205,0.12)] border border-slate-100/90">
            ...
            <div class="text-center mb-4">
                <div class="w-14 h-14 bg-sky-50 text-primary-600 rounded-2xl mx-auto flex items-center justify-center mb-3 border border-sky-100 shadow-sm">
                ...
                <div class="mt-2 inline-flex items-center gap-1.5 px-3 py-1 bg-sky-50 border border-sky-200/80 rounded-full text-xs font-extrabold text-primary-600">
                ...
                <form method="POST" action="{{ route('verification.otp.verify') }}" class="space-y-4">
                    ...
                    <div class="flex justify-center items-center gap-1.5 sm:gap-2.5 py-2">
                        <template x-for="(digit, index) in digits" :key="index">
                            <input type="text"
                                   ...
                                   class="w-10 h-12 sm:w-12 sm:h-14 text-center text-xl font-black text-[#0F2747] bg-slate-50 border-2 rounded-xl focus:bg-white focus:border-primary-600 focus:ring-4 focus:ring-[#009CA6]/15 transition-all outline-none"
                                   :class="digits[index] ? 'border-primary-600 bg-emerald-50/20' : 'border-slate-200'"
                                   autocomplete="off">
                        </template>
                    </div>
                    ...
                    <div>
                        <button type="submit"
                                ...
                                class="w-full py-3 px-5 rounded-xl bg-primary-600 hover:bg-primary-700 disabled:bg-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed text-white font-extrabold text-xs sm:text-sm shadow-md shadow-[#009CA6]/20 transition-all duration-200 transform active:scale-[0.99] flex items-center justify-center tracking-wider uppercase cursor-pointer">
                            VERIFIKASI KODE OTP
                        </button>
                    </div>
                </form>
                ...
                <div class="mt-4 pt-4 border-t border-slate-100 text-center">
                    <form method="POST" action="{{ route('verification.otp.resend') }}">
                        ...
                        <button type="submit"
                                ...
                                :class="resendTimer > 0 ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-sky-50 hover:bg-sky-100 text-primary-600 hover:text-[#007b83] border border-sky-200 cursor-pointer'">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="mt-4 text-center">
                <a href="{{ route('login') }}" class="text-xs font-semibold text-slate-500 hover:text-primary-600 transition-colors inline-flex items-center gap-1">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
